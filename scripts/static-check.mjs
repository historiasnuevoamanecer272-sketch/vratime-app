...
if (!favicon.includes('<title>VratiMe</title>') || !favicon.includes('#087783')) throw new Error('Brand favicon is missing');
const [ogWidth, ogHeight] = await pngSize(path.join(dist, 'og.png'));
if (ogWidth !== 1200 || ogHeight !== 630) throw new Error(`Open Graph image must be 1200x630, got ${ogWidth}x${ogHeight}`);
...
for (const [size, file] of expectedIcons) {
  ...
  const [width, height] = await pngSize(path.join(dist, file));
  if (width !== size || height !== size) throw new Error(`PNG dimensions do not match ${size}x${size}`);
}
